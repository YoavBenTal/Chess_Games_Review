        ...
        if maybe_hanging_piece is not None:
            ...
            for defending_square in chess.SQUARES:  # see if attacked piece has defenders
                ...
                if (maybe_defending_piece is not None) and (maybe_hanging_piece.color == maybe_defending_piece.color):
                    ...
                    if hanging_square in list(board.attacks(
                            defending_square)):  # check if attacked piece's square is on the attacks (is defended by) of another piece
        ...
        if (maybe_hanging_piece is not None):
            if not (is_defended(board, square)):
                ...
                if len(attackers) > 0:
                    ...
                    if fr_format:
                        hanging_pieces_and_attackers[chess.square_name(square)] = [chess.square_name(s) for s in
                                                                                   attackers]
...
def evaluate(board, return_mate_n=False):
    ...
    if board.turn == True:
        ...
        score = -info[
            'score'].relative.score()  # transform score to absolute score (pos in favor of white, neg in favor black)
...
def is_trapped(board: chess.Board, square, by):
    ...
    for move_to_square in movable_squares:
            ...
            for defending_square in defending_squares:
                ...
                if board.piece_at(defending_square).color != board.piece_at(square).color:
                    ...
                    if board.piece_type_at(defending_square) < board.piece_type_at(square):
                        ...
                        if not board.is_pinned(by, defending_square):
                            defenders = is_defended(board, defending_square, by_color=not by,
        ...
        elif (board.piece_at(move_to_square).color != board.piece_at(square).color) and (
                board.piece_type_at(move_to_square) <= board.piece_type_at(square)):
...
def move_pins_opponent(board: chess.Board, move, return_pinned_square=False):
    ...
    for square in possible_pinned_squares:
        if (position_after_move.piece_at(square) is not None) and (
                position_after_move.piece_at(square).color == position_after_move.turn):
...
def move_attacks_piece(board: chess.Board, move: chess.Move, return_attacked_piece=False):
    ...
    if is_defended(position_after_move, move.to_square) or not board.is_attacked_by(position_after_move.turn,
                                                                                    move.to_square):
        attacked_squares = list(position_after_move.attacks(move.to_square))
        for attacked_square in attacked_squares:
            if position_after_move.piece_at(attacked_square) is not None:
                if str(position_after_move.piece_at(attacked_square)).lower() != 'k':
                    if position_after_move.piece_at(attacked_square).color != position_after_move.piece_at(
                            move.to_square).color:
                        if position_after_move.piece_type_at(attacked_square) > position_after_move.piece_type_at(
                                move.to_square):

                            if return_attacked_piece:
                                return position_after_move.piece_at(attacked_square)
                            return True
                        elif is_hanging(position_after_move, attacked_square,
...
def parse_time_stamps(pgn):
    """Extracts time control from a Chess.pgn.Game object."""

    # Check if time control is available
    time_increment = None
    starting_time = None
    if hasattr(pgn, "time_control") and pgn.time_control() and pgn.time_control().parts:
        starting_time = getattr(pgn.time_control().parts[0], "time", None)
        time_increment = getattr(pgn.time_control().parts[0], "increment", None)

    # Ensure there are moves
    if not pgn.variations:
        return [], time_increment, starting_time

    time_stamps_list = []
    node = pgn.variations[0]

    while node is not None:
        match = CLOCK_REGEX.search(getattr(node, "comment", ""))
        if match:
            hours = int(match.group("hours"))
            minutes = int(match.group("minutes"))
            seconds = float(match.group("seconds"))
            time_stamps_list.append(hours * 3600 + minutes * 60 + seconds)
        else:
            time_stamps_list.append(None)

        # Move to the next node safely
        node = node.variations[0] if node.variations else None

    return time_stamps_list, time_increment, starting_time
...
def parse_pgn(pgn, san_only=False):
    ...
    time_stamps_list, time_increment, starting_time = parse_time_stamps(pgn)
    ...
    if san_only:
        ...
        return uci_moves, san_moves, fens, time_stamps_list, time_increment, starting_time
...
def get_best_move(board: chess.Board):
    ...
    if 'pv' in info:
        best_move = info['pv'][0]
        return best_move
    else: # There are no legal moves
        return chess.Move.null()  # return dummy value
...
def review_move(board: chess.Board, move, previous_review: str, check_if_opening=False):
    ...
    if move_classication in ['excellent', 'good']:
        ...
        if ('creates a fork' not in previous_review) or (not board.is_check()) or ('trade' not in previous_review) or (
                'lower value' not in previous_review):
            ...
            possible_hanging_squares = [s for s in possible_hanging_squares if
                                        position_after_move.piece_at(s).color == board.turn]
            if len(possible_hanging_squares) > 0:
                hanging_squares = [chess.square_name(s) for s in possible_hanging_squares]
                hanging_pieces = [piece_dict[str(position_after_move.piece_at(s)).lower()] for s in
                                  possible_hanging_squares]
        ...
        if (len(capturable_pieces_by_lower) > 0) and (not position_after_move.is_check()) and (
        not is_possible_trade(board, move)):
            capturable_pieces_by_lower = [piece_dict[str(position_after_move.piece_at(s)).lower()] for s in
                                          capturable_pieces_by_lower]
        ...
        attacked_squares_with_check = move_is_discovered_check_and_attacks(position_after_move, lets_opponent_play_move,
        ...
        missed_attacked_squares_with_check = move_is_discovered_check_and_attacks(board, best_move,
                                                                                  return_attacked_squares=True)
        ...
        if not (len(attacked_squares_with_check) > 0):
            trapped_squares = move_traps_opponents_piece(position_after_move, lets_opponent_play_move,
...
def review_game(uci_moves, roast=False, verbose=False):
    ...
    for i, move in enumerate(tqdm(uci_moves)):
        ...
        if roast:
            classification, review, uci_best_move, san_best_move = roast_move(board, move, previous_review,
                                                                              check_if_opening)
        else:
            classification, review, uci_best_move, san_best_move = review_move(board, move, previous_review,
                                                                               check_if_opening)
